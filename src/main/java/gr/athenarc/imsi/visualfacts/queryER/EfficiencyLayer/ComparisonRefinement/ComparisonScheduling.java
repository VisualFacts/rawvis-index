...
import gr.athenarc.imsi.visualfacts.queryER.Comparators.ComparisonUtilityComparator;
...
public class ComparisonScheduling extends AbstractEfficiencyMethod {
    ...
    private void setComparisonUtilities() {
        ...
        if (weighted) {
            ...
            for (Comparison comparison : comparisons) {
                double inverseCompFreq1 = Math.log(noOfComparisons / comparisonsPerEntity[(int)comparison.getEntityId1()]);
                if (maxFreq1 < inverseCompFreq1) {
                    maxFreq1 = inverseCompFreq1;
                }

                int entityId2 = (int)  comparison.getEntityId2()+entityIndex.getDatasetLimit();
