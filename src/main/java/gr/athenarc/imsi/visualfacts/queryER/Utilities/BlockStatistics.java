...
import gr.athenarc.imsi.visualfacts.queryER.DataStructures.GroundTruthEntityIndex;
...
public class BlockStatistics implements Constants {
    ...
    private boolean areCooccurring(boolean cleanCleanER, IdDuplicates pairOfDuplicates) {
        long[] blocks1 = entityIndex.getEntityBlocks(pairOfDuplicates.getEntityId1(), 0);
        if (blocks1 == null) {
            return false;
        }
        
        long[] blocks2 = entityIndex.getEntityBlocks(pairOfDuplicates.getEntityId2(), cleanCleanER?1:0);
        if (blocks2 == null) {
            return false;
        }
    ...
    private void getDecomposedBlocksEntities(double totalComparisons) throws IOException {
        DecomposedBlock deBlock = (DecomposedBlock) blocks.get(0);
        final Set<Long> entitiesD1 = new HashSet<Long>((int)totalComparisons);
        if (deBlock.isCleanCleanER()) {
            final Set<Long> entitiesD2 = new HashSet<Long>((int)totalComparisons);
            for (AbstractBlock block : blocks) {
    ...
    private void getEntities() throws IOException {
        if (blocks.get(0) instanceof UnilateralBlock) {
            Set<Long> distinctEntities = new HashSet<Long>();
            for (AbstractBlock block : blocks) {
                UnilateralBlock uBlock = (UnilateralBlock) block;
                for (long entityId : uBlock.getEntities()) {
            ...
            Set<Long> distinctEntitiesD1 = new HashSet<Long>();
            Set<Long> distinctEntitiesD2 = new HashSet<Long>();
            for (AbstractBlock block : blocks) {
                BilateralBlock bBlock = (BilateralBlock) block;
                for (long entityId : bBlock.getIndex1Entities()) {
                    distinctEntitiesD1.add(entityId);
                }
                for (long entityId : bBlock.getIndex2Entities()) {
