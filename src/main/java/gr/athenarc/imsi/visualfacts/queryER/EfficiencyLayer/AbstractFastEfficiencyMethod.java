...
public abstract class AbstractFastEfficiencyMethod extends AbstractEfficiencyMethod {
    ...
    protected void addDecomposedBlock(int entityId, Collection<Long> neighbors, List<AbstractBlock> newBlocks) {
        if (neighbors.isEmpty()) {
            return;
        }

        long[] entityIds1 = replicateId(entityId, neighbors.size());
        long[] entityIds2 = Converter.convertCollectionToArray(neighbors);
        newBlocks.add(new DecomposedBlock(cleanCleanER, entityIds1, entityIds2));
    }
    
    protected void addReversedDecomposedBlock(int entityId, Collection<Long> neighbors, List<AbstractBlock> newBlocks) {
        if (neighbors.isEmpty()) {
            return;
        }

        long[] entityIds1 = Converter.convertCollectionToArray(neighbors);
        long[] entityIds2 = replicateId(entityId, neighbors.size());
        newBlocks.add(new DecomposedBlock(cleanCleanER, entityIds1, entityIds2));
    }
    ...
    protected long[] replicateId(int entityId, int times) {
        int counter = 0;
