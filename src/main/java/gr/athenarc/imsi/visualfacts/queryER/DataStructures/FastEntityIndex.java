...
public class FastEntityIndex implements Serializable {
    ...
    private void firstPass(List<AbstractBlock> blocks) {
        ...
        if (cleanCleanER) {
            ...
            for (AbstractBlock block : blocks) {
                ...
                for (long id1 : bBlocks[counter].getIndex1Entities()) {
                    if (noOfEntities < id1 + 1) {
                        noOfEntities = (int) (id1 + 1);
                    }
                }

                for (long id2 : bBlocks[counter].getIndex2Entities()) {
            ...
            for (AbstractBlock block : blocks) {
                ...
                for (long id : uBlocks[counter].getEntities()) {
    ...
    private void indexBilateralEntities() {
        ...
        for (BilateralBlock block : bBlocks) {
            ...
            for (long id1 : block.getIndex1Entities()) {
                counters[(int) id1]++;
                entityComparisons[(int) id1] += innerSize2;
            }

            for (long id2 : block.getIndex2Entities()) {
        ...
        for (BilateralBlock block : bBlocks) {
            for (long id1 : block.getIndex1Entities()) {
                entityBlocks[(int) id1][counters[(int) id1]] = counter;
                counters[(int) id1]++;
            }
    ...
    private void indexUnilateralEntities() {
        ...
        for (UnilateralBlock block : uBlocks) {
            ...
            for (long id : block.getEntities()) {
                counters[(int) id]++;
                entityComparisons[(int) id] += blockSize-1;
            }
        }
        ...
        for (UnilateralBlock block : uBlocks) {
            for (long id : block.getEntities()) {
                entityBlocks[(int) id][counters[(int) id]] = counter;
                counters[(int) id]++;
            }
            counter++;
        }
